...
class HbAdminPageRules extends HbAdminPage {
	...
	<div class="wrap">
		...
		<h3 id="hb-additional-rules-title">
			<?php esc_html_e( 'Advanced booking rules', 'hbook-admin' ); ?>
		...
		<p><?php esc_html_e( 'You may define advanced rules to create compulsory settings for stays, special rates, or discounts.', 'hbook-admin' ); ?></p>
		...
		<table class="wp-list-table widefat hb-conditional-rule-table">
			...
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'If check-in is on a:', 'hbook-admin' ); ?></th>
					<th><?php esc_html_e( 'then check-out is on a:', 'hbook-admin' ); ?></th>
					<th><?php esc_html_e( 'and minimum stay is:', 'hbook-admin' ); ?> <span class="hb-rule-legend"><?php esc_html_e( '(leave empty if not applicable)', 'hbook-admin' ); ?></span></th>
					<th><?php esc_html_e( 'and maximum stay is:', 'hbook-admin' ); ?> <span class="hb-rule-legend"><?php esc_html_e( '(leave empty if not applicable)', 'hbook-admin' ); ?></span></th>
					<th><?php esc_html_e( 'for accommodation:', 'hbook-admin' ); ?></th>
					<th class="hb-rule-box-with-border" width="11%"><?php esc_html_e( 'for season:', 'hbook-admin' ); ?></th>
		...
		<script id="conditional_rule_text_tmpl" type="text/html">
			<tr>
				...
				<td data-bind="text: accom_list"></td>
				<td data-bind="text: seasons_list" class="hb-rule-box-with-border"></td>
		...
		<script id="conditional_rule_edit_tmpl" type="text/html">
			<tr>
				<td class="hb-rule-box-with-border"><input data-bind="value: name" type="text" /></td>
				<td><?php $this->display_select_days( 'check_in_days' ); ?></td>
				<td><?php $this->display_select_days( 'check_out_days' ); ?></td>
				<td>
					<input data-bind="value: minimum_stay" class="hb-stay-length" type="text" /> <span><?php esc_html_e( 'nights', 'hbook-admin' ); ?></span>
				</td>
				<td>
					<!-- ko if: maximum_stay() == 9999 -->
					---
					<!-- /ko -->
					<!-- ko if: maximum_stay() != 9999 -->
					<input data-bind="value: maximum_stay" class="hb-stay-length" type="text" /> <span><?php esc_html_e( 'nights', 'hbook-admin' ); ?></span>
					<!-- /ko -->
				</td>
				<td><?php $this->display_checkbox_list( $this->accom, 'accom' ); ?></td>
				<td class="hb-rule-box-with-border"><?php $this->display_checkbox_list( $this->seasons, 'seasons' ); ?></td>
				<td class="hb-rule-box-with-border">
					<?php 
					foreach ( $this->conditional_types as $cond_type => $cond_type_label ) { 
						$cond_type_id = 'hb' . $cond_type;
						?>
