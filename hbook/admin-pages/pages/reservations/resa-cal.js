...
function hb_set_resa_cal( all_resa, all_blocked_accom, all_customers, day, displayed_accoms ) {
	...
	jQuery.each( displayed_accoms, function( accom_id, accom ) {
        jQuery.each( accom.num_name, function( accom_num, num_name ) {
            cal_mark_up += '<tr>' + get_day_tds( 'days-cal', day, accom_id, accom_num ) + '</tr>';
        });
    });

	var td_width = jQuery( '.hbd' ).outerWidth();
	...
	for ( var i = 0; i < all_blocked_accom.length; i++ ) {
		if ( hb_dates_in_range( all_blocked_accom[i].from_date(), all_blocked_accom[i].to_date() ) ) {
	...
	jQuery( '#hb-resa-cal-table' ).html( '<tr>' + get_day_tds( 'days-table', day ) + '</tr>' + cal_mark_up + '<tr id="hb-cal-days-bottom">' + get_day_tds( 'days-table', day ) + '</tr>' );
}
